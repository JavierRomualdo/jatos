...
export class LocalesComponent implements OnInit {
  ...
  mostrarprovincias(idubigeo) {
    if (idubigeo > 0) {
      ...
      // this.mostrarlocales(departamento.tipoubigeo_id, departamento.codigo);
      this.mostrarlocales();
    } else {
      this.locales = [];
      this.ubigeoprovincias = [];
  ...
  mostrarlocales() {
    this.cargando = true;
    this.api.post2('mostrarlocales', this.ubigeo).then(
      (res) => {
        this.locales = res;
        console.log('Ubigeo:');
        console.log(this.ubigeo);
        console.log('Los locales son:');
        console.log(res);
        this.cargando = false;
      },
      (error) => {
  ...
  verDetalleLocal(idlocal: number) {
    this._router.navigate(['/welcome/local/detalle', idlocal]);
  }
