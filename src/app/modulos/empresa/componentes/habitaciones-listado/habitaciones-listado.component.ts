...
export class HabitacionesListadoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.localeText = { noRowsToShow: 'No se encontraron habutaciones', page: "Página", of: "de", to: "a" };
    if (this.isModal) {
  ...
  nuevaHabitacion() {
    ...
    const ids = this.listadoHabitaciones.map(casa => casa.id);
    const idMax = Math.max(...ids);
    const ultimaHabitacion = this.listadoHabitaciones.find(casa => casa.id == idMax);
    this.emitirAccion(LS.ACCION_NUEVO, ultimaHabitacion);
  }
  ...
  imprimirDetalleHabitacion() {
    this.cargando = true;
    let parametros = {
      habitacion: this.objetoSeleccionado,
      fechaActual: this.utilService.obtenerFechaActual()
    }
    this.habitacionService.imprimirHabitacionDetalle(parametros, this);
  }
  ...
  iniciarAgGrid() {
    ...
    this.rowClassRules = this.habitacionService.generarReglaPaFilasConMensajes();
